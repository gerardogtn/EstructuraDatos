...
// REQUIRES: Graph is non-directed graph.
// MODIFIES: None.
// EFFECTS : Returns true if the node is complete (all nodes have connections to all other nodes).
template <class V, class E>
bool Graph<V, E>::isComplete() const{
    bool output = false;
    
    for (int i = 0; i < nodes.size() - 1; i++){
        
        for (auto v : * nodes[i]->getEdges()) {
            
            for (int j = i + 1; j < nodes.size(); j++) {
                
                for (auto v2 : * nodes[j]->getEdges()){
                    if (v2 == v) {
                        output = true;
                    }
                }
